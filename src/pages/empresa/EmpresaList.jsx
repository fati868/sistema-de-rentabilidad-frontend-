...
const EmpresaList = () => {
  ...
  const ownerOf = (id) => owners.find((o) => o.id_empresa === id);
  ...
  return (
    <Layout>
      <div className="animate-fadeInUp">
        ...
        <div className="page-header d-flex justify-content-between align-items-start flex-wrap gap-3">
          ...
          <button className="btn btn-primary d-flex align-items-center gap-2 px-4"
            onClick={() => { setEmpresaId(null); setShowModal(true); }}>
        ...
        <div className="row g-3 mb-4 stagger">
          {[
            { label: "Total empresas", value: empresas.length, icon: "bi-building-fill", color: "var(--primary)", bg: "rgba(79,70,229,.1)" },
            { label: "Con propietario", value: empresas.filter((e) => ownerOf(e.id_empresa)).length, icon: "bi-person-check-fill", color: "var(--success)", bg: "rgba(16,185,129,.1)" },
            { label: "Sin propietario", value: empresas.filter((e) => !ownerOf(e.id_empresa)).length, icon: "bi-person-x-fill", color: "var(--warning)", bg: "rgba(245,158,11,.1)" },
          ].map((s, i) => (
        ...
        {/* Cards grid — visualización moderna con propietario */}
        {loading ? (
            ...
            {filtered.map((empresa) => {
              ...
              return (
                <div className="col-12 col-md-6 col-lg-4" key={empresa.id_empresa}>
                  <div className="card border-0 rounded-4 h-100 card-3d animate-fadeInUp overflow-hidden"
                    ...
                    <div className="p-4">
                      ...
                      <div className="d-flex align-items-start justify-content-between mb-3">
                        <div className="d-flex align-items-center gap-3">
                          <div className="rounded-3 d-flex align-items-center justify-content-center"
                            style={{ width: 44, height: 44, background: "rgba(79,70,229,.1)", flexShrink: 0 }}>
                            <i className="bi bi-building-fill" style={{ color: "var(--primary)", fontSize: 20 }}></i>
                          </div>
                          <div>
                            <h6 className="fw-bold mb-0">{empresa.nombre}</h6>
                            <span className="text-muted" style={{ fontSize: 12 }}>ID #{empresa.id_empresa}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="rounded-3 p-3 mb-3"
                        style={{ background: owner ? "rgba(16,185,129,.06)" : "rgba(245,158,11,.06)" }}>
                        {owner ? (
                          <div className="d-flex align-items-center gap-2">
                            ...
                            <div>
                              <p className="fw-semibold mb-0" style={{ fontSize: 13 }}>{owner.nombre}</p>
                              <p className="text-muted mb-0" style={{ fontSize: 11 }}>{owner.email}</p>
                            </div>
                            <span className="badge badge-role badge-active ms-auto">Owner</span>
                          </div>
                        ) : (
                          <div className="d-flex align-items-center gap-2 text-warning">
                            <i className="bi bi-exclamation-triangle-fill"></i>
                            <span style={{ fontSize: 12 }} className="fw-medium">Sin propietario asignado</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="d-flex gap-2">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setEditingOwner(ownerOf(empresa.id_empresa) || null);
                            setShowModal(true);
                          }}>
      ...
      <EmpresaForm
        show={showModal}
        onClose={() => { setShowModal(false); setEditingOwner(null); }}
        onSuccess={fetchData}
        empresaId={empresaId}
        owner={editingOwner}
      />
